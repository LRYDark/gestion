...
function update_150_remote() {
   ...
   $DB->doQueryOrDie($create_remote_sign_requests, "Erreur lors de la création de la table glpi_plugin_gestion_remote_sign_requests");
   $DB->doQueryOrDie($create_signaturedevices, "Erreur lors de la création de la table glpi_plugin_gestion_signaturedevices");
   ...
   try {
      $r = $DB->doQuery("SHOW VARIABLES LIKE 'event_scheduler'");
      $current = null;
   ...
   $dbName = null;
   if ($r = $DB->doQuery("SELECT DATABASE() AS db")) {
      if ($row = $r->fetch_assoc()) { $dbName = $row['db']; }
   }
   if (!$dbName) { $dbName = 'glpi'; }

   // Recrée l'event proprement
   $DB->doQuery("DROP EVENT IF EXISTS `ev_cleanup_remote_sign_requests`");
