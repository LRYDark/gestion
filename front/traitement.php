...
try {
    ...
    $chk = $DB->query("SHOW COLUMNS FROM `$table` LIKE 'tech_ext'");
    if ($chk && $DB->numrows($chk) === 0) {
        // Best-effort: add column if not exists (safe no-op if lacks perms)
        @$DB->query("ALTER TABLE `$table` ADD COLUMN `tech_ext` VARCHAR(255) NULL AFTER `users_ext`");
    }
} catch (Throwable $e) {
    // ignore
}
...
if (empty($_POST['email'])) $_POST['email'] = "vide";
$EMAIL = $_POST["email"];
